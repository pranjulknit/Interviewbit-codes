...
// Position of middle node is: (N/2)+1, where N is the total number of nodes
//  in the list.
...
//  The Middle of the linked list is the node with value 16.
//  There are only 3 nodes to the left of the middle node and we need to find the 10th node which doesn't exist
//   so we will return -1.
...
int Solution::solve(ListNode* A, int B) {
    ...
    // fast pointer should make ahead of B steps
    // so that fast pointer when ends
    // slow should at kth node from middile
    
    
    // because in B step fast pointer will 
    // move 2*B steps;
